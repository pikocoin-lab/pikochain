...
**命题 7（最优查询费）。** 设潜在查询总量为 $M$，单次查询价值 $v \sim G$（密度 $g$，支撑 $[0, V]$），查询者当且仅当 $v \geq f$ 时购买。则：
...
本文把一句话变成了可计算的机制：**电力是真实的钱，电价差是真实的利润，知识是电力的结晶，查询是知识的变现**。四个公式分别回答了：谁来生产（入场条件）、生产什么质量（凸性奖励）、系统如何不亏钱（校准铁律）、币价波动时怎么办（难度调整）。这套机制不需要知道矿工在哪里，不需要验证每一度电，只需要验证知识的质量和查询的真实——而这两者，恰好是 AI 自己最擅长验证的东西。
